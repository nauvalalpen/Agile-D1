<form action="{{ route('admin.gallery.edit', $gallery->id) }}" method="POST" enctype="multipart/form-data" id="editGalleryForm{{ $gallery->id }}">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <textarea
            ...
            id="deskripsi{{ $gallery->id }}"
            name="deskripsi"
            rows="3"
            placeholder="Tuliskan deskripsi singkat"
            required>{{ old('deskripsi', $gallery->deskripsi) }}</textarea>
        @error('deskripsi')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="mb-3 d-none" id="imagePreview{{ $gallery->id }}">
        ...
        <img src="" alt="Preview" class="img-thumbnail" style="max-width: 100%; height: auto; max-height: 200px;">
    </div>
